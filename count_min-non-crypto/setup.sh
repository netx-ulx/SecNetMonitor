...
def setup(mode):
	...
	if mode == "src_ip":
		os.system('../p4c-bmv2/p4c_bm/__main__.py p4src/count_min.p4 --json count_min.json')
		...
		os.system('../p4c-bmv2/p4c_bm/__main__.py p4src/count_min-flow.p4 --json count_min-flow.json')
		os.system('sudo ../bmv2/targets/simple_switch/simple_switch >/dev/null 2>&1')
	
		cmd = "sudo python topo.py \
			--behavioral-exe ../bmv2/targets/simple_switch/simple_switch \
			--json count_min-flow.json \
			--cli ../bmv2/tools/runtime_CLI.py"
...
def wrong_args_msg():
	print "Usage: setup.py [--flow]"

if __name__ == '__main__':
	argc = len(sys.argv)
	if argc == 1:
		setup("src_ip")
	elif argc == 2 and sys.argv[1] == "--flow":
		setup("flow")
	else:
		wrong_args_msg()
